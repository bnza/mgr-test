...
export abstract class BaseDataPage extends BasePage {
    ...
    public readonly dataDialogCloseButton = this.page.getByTestId('data-dialog-form-close-button')
    public readonly dataDialogSubmitButton = this.page.getByTestId('data-dialog-form-submit-button')
    ...
    async expectDayaDialogTextFieldToHaveValue(name: string, value: string | RegExp) {
        await expect(this.dataDialogForm.getByRole('textbox', {name})).toHaveValue(value)
    }
}